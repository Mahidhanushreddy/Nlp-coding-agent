...
import { AgentConfig, VSCodeContext, ResponseSections } from './types';
...
let nlpAgent: NLPAgent;
// Dedicated terminal for all executions
let nlpAgentTerminal: vscode.Terminal | undefined;

function getOrCreateNlpAgentTerminal(): vscode.Terminal {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  nlpAgent = new NLPAgent(context, defaultConfig);
...
async function executeCommandInTerminal(command: string): Promise<void> {
  try {
...
async function executeProgramInTerminal(filePath: string, language: string): Promise<void> {
  try {
    const terminal = getOrCreateNlpAgentTerminal();
...
async function createAndRunFile(fileName: string, content: string, language: string): Promise<void> {
  try {
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!workspaceRoot) {
